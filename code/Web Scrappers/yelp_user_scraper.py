...
revew_url="https://www.yelp.com/user_details_reviews_self?"
header_row=['user_name','user_location','user_tagline','friend_count','review_count','photo_count','five_start_count','four_start_count',
            'three_start_count','two_start_count','one_start_count','useful_review_count','funny_review_count','cool_review_count','stats_dict',
            'compliments_number','yelping_since','other_info_dict','elite_status_count','user_cities_dict']

##def load_Users_Dict():

  ##Read User ID data from file
  ##with open('user_ids.csv','rU') as readfile:
  ##  reader = csv.reader(readfile)
  ##  for row in reader:
  ##        i=i+1
  ##    user_ids_dict[i]=row[0]
  ...
  for u_id in user_ids_dict:
    ...
    if soup.find("div","error-wrap") or not soup.find("div","user-profile_container"): 
      ...
      for url in url_review_list:
        ...
        if not review_soup: ##Add condition to check for no more reviews page
          break
        location=review_soup.findAll("address")
        user_cities.extent([loc.text for loc in location])
        print(user_cities)
      
      ##CONSTRUCT USER ROWS - SEND EACH ROW TO file
      review_data_row = [user_name,user_location,user_tagline,friend_count,review_count,photo_count,five_start_count,four_start_count,
                        three_start_count,two_start_count,one_start_count,useful_review_count,funny_review_count,cool_review_count,stats_dict,
                        compliments_number,yelping_since,other_info_dict,elite_status_count,user_cities_dict]
      writer.writerow(review_data_array) [string.encode('utf-8','xmlcharrefreplace') for string in review_data_row]
