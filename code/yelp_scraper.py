...
'''
  ...
  #SCRAPE DATA
  with open('business_urls.csv','rb') as readfile:
    reader = csv.reader(readfile, delimiter=',')
    for row in reader:
      businesses_dict = {} #populate from API
      url_list = []
      url_list.extend([for x in range() str(row[2])])
      businesses_dict.update({str(row[0]):url_list})

  for b_id,urls in businesses_dict: #expand business urls, getting each page of 20 reviews
  ...
  for business_id,url in businesses_dict:
